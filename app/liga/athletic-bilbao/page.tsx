...
export default function AthleticBilbaoPage() {
  const joueurs = generatePlayers('Athletic Bilbao', 15);

  return (
    <main className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        <Link href="/liga" className="text-blue-900 hover:underline mb-4 inline-block">
          ← Retour à la Liga
        </Link>
        <h1 className="text-4xl font-bold text-blue-900 mb-8">
          Effectif Athletic Bilbao
        </h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {joueurs.map(joueur => (
            <div key={joueur.id} className="bg-white p-6 rounded-lg shadow-md">
              ...
              <Link 
                href={`/liga/athletic-bilbao/${joueur.id}`}
