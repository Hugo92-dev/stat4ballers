...
# IDs des 20 équipes de Premier League 2025/2026
PREMIER_LEAGUE_TEAMS = [
    {'id': 19, 'name': 'Arsenal', 'slug': 'arsenal'},
    {'id': 15, 'name': 'Aston Villa', 'slug': 'aston-villa'},
    {'id': 52, 'name': 'AFC Bournemouth', 'slug': 'bournemouth'},
    {'id': 236, 'name': 'Brentford', 'slug': 'brentford'},
    {'id': 78, 'name': 'Brighton & Hove Albion', 'slug': 'brighton'},
    {'id': 27, 'name': 'Burnley', 'slug': 'burnley'},
    {'id': 18, 'name': 'Chelsea', 'slug': 'chelsea'},
    {'id': 51, 'name': 'Crystal Palace', 'slug': 'crystal-palace'},
    {'id': 13, 'name': 'Everton', 'slug': 'everton'},
    {'id': 11, 'name': 'Fulham', 'slug': 'fulham'},
    {'id': 71, 'name': 'Leeds United', 'slug': 'leeds'},
    {'id': 8, 'name': 'Liverpool', 'slug': 'liverpool'},
    {'id': 9, 'name': 'Manchester City', 'slug': 'manchester-city'},
    {'id': 14, 'name': 'Manchester United', 'slug': 'manchester-united'},
    {'id': 20, 'name': 'Newcastle United', 'slug': 'newcastle'},
    {'id': 63, 'name': 'Nottingham Forest', 'slug': 'nottingham-forest'},
    {'id': 3, 'name': 'Sunderland', 'slug': 'sunderland'},
    {'id': 6, 'name': 'Tottenham Hotspur', 'slug': 'tottenham'},
    {'id': 1, 'name': 'West Ham United', 'slug': 'west-ham'},
    {'id': 29, 'name': 'Wolverhampton Wanderers', 'slug': 'wolves'}
]
    ...
    try:
        ...
        for player_data in squad:
            if 'player' in player_data and player_data['player']:
                ...
                if player.get('id') == 28931574:  # João Pedro de Chelsea
                    name = "João Pedro"
                elif player.get('id') == 37701999:  # Estêvão de Chelsea
                    name = "Estêvão"
                elif common_name:
                    name = common_name
                elif display_name:
...
def generate_typescript_data(teams_data):
    ...
    ts_content = """// Données des équipes de Premier League - Saison 2025/2026
...
def main():
    print("=" * 60)
    print("RÉCUPÉRATION DES EFFECTIFS PREMIER LEAGUE - 2025/2026")
    print("=" * 60)
    print(f"\nRécupération de {len(PREMIER_LEAGUE_TEAMS)} équipes...\n")
    
    # Trier les équipes par ordre alphabétique
    PREMIER_LEAGUE_TEAMS.sort(key=lambda x: x['name'])
    ...
    if all_teams_data:
        ...
        output_file = "data/premierLeagueTeams.ts"
        ...
        print(f"RÉSUMÉ")
        print(f"{'='*60}")
        print(f"✅ {len(all_teams_data)}/{len(PREMIER_LEAGUE_TEAMS)} équipes récupérées avec succès")
        ...
        for team, players in all_teams_data:
            if team['slug'] == 'arsenal':
                for player in players:
                    if 'Saka' in player['name']:
                        print(f"  Arsenal - {player['name']} ({player['nationality']})")
                        break
        
        # Vérifier Liverpool
        for team, players in all_teams_data:
            if team['slug'] == 'liverpool':
                for player in players:
                    if 'Salah' in player['name']:
                        print(f"  Liverpool - {player['name']} ({player['nationality']})")
                        break
        
        # Vérifier Man City
        for team, players in all_teams_data:
            if team['slug'] == 'manchester-city':
                for player in players:
                    if 'Haaland' in player['name']:
                        print(f"  Man City - {player['name']} ({player['nationality']})")
                        break
    else:
