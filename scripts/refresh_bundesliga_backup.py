...
# -*- coding: utf-8 -*-
"""
Script de refresh pour la Bundesliga
...
SEASON_ID = 25646  # Saison 2025/2026
OUTPUT_DIR = "bundesliga_2025_2026"

# Clubs de Bundesliga avec leurs IDs et slugs corrects
BUNDESLIGA_TEAMS = {
    503: "bayern-munich",
    68: "borussia-dortmund",
    277: "rb-leipzig",
    3321: "bayer-leverkusen",
    1079: "union-berlin",
    3543: "freiburg",
...
def refresh_bundesliga():
    """Rafraîchit toutes les données de la Bundesliga"""
    
    print("=" * 60)
    print(f"🇩🇪 REFRESH BUNDESLIGA - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    ...
    print(f"\n📊 Récupération de {len(BUNDESLIGA_TEAMS)} équipes de Bundesliga...")
    
    # Traiter chaque équipe
    for team_id, team_slug in BUNDESLIGA_TEAMS.items():
        ...
        if team_id != list(BUNDESLIGA_TEAMS.keys())[-1]:
            time.sleep(2)
    ...
    print("\n" + "=" * 60)
    print("📈 RÉSUMÉ DU REFRESH BUNDESLIGA")
    print("=" * 60)
    print(f"✅ Équipes mises à jour: {success_count}/{len(BUNDESLIGA_TEAMS)}")
    print(f"❌ Erreurs: {error_count}/{len(BUNDESLIGA_TEAMS)}")
    ...
    print(f"\n✨ Refresh Bundesliga terminé!")
    
    return success_count == len(BUNDESLIGA_TEAMS)

if __name__ == "__main__":
    try:
        success = refresh_bundesliga()
