...
def get_player_info(player_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Erreur récupération info joueur {player_id}: {e}")
...
def get_player_season_stats(player_id: int, season_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Erreur récupération stats joueur {player_id} saison {season_id}: {e}")
    
    return None

def get_om_squad(season_id: int) -> list:
    """Récupère la liste des joueurs de l'OM pour une saison"""
    url = f"{BASE_URL}/squads/teams/{OM_TEAM_ID}/extended"
    ...
    try:
        ...
        print(f"Erreur récupération effectif OM saison {season_id}: {e}")
...
def main():
    ...
    # Récupérer l'effectif actuel (2025/2026)
    current_season_id = SEASON_IDS["2025/2026"]
    print(f"\n📋 Récupération de l'effectif OM saison 2025/2026...")
    
    squad = get_om_squad(current_season_id)
    ...
    for player_info in squad:
        ...
        for season_name, season_id in SEASON_IDS.items():
            print(f"  📊 Saison {season_name}...", end=" ")
            ...
            if stats and stats.get("appearences", 0) > 0:
                ...
                print(f"✅ {stats['appearences']} matchs, {stats['goals']} buts, {stats['assists']} passes")
            else:
                print("⚫ Pas de données ou 0 match")
        
        # Ajouter le joueur aux données
        if player_data["stats"]:  # Seulement si le joueur a des stats
