...
# -*- coding: utf-8 -*-
"""
Script de refresh pour la Premier League
...
SEASON_ID = 25583  # Saison 2025/2026
OUTPUT_DIR = "premier-league_2025_2026"

# Clubs de Premier League avec leurs IDs et slugs corrects
...
def refresh_premier_league():
    """Rafraîchit toutes les données de la Premier League"""
    
    print("=" * 60)
    print(f"🏴󠁧󠁢󠁥󠁮󠁧󠁿 REFRESH PREMIER LEAGUE - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    ...
    print(f"\n📊 Récupération de {len(PREMIER_LEAGUE_TEAMS)} équipes de Premier League...")
    
    # Traiter chaque équipe
    for team_id, team_slug in PREMIER_LEAGUE_TEAMS.items():
        ...
        if team_id != list(PREMIER_LEAGUE_TEAMS.keys())[-1]:
            time.sleep(2)
    ...
    print("\n" + "=" * 60)
    print("📈 RÉSUMÉ DU REFRESH PREMIER LEAGUE")
    print("=" * 60)
    print(f"✅ Équipes mises à jour: {success_count}/{len(PREMIER_LEAGUE_TEAMS)}")
    print(f"❌ Erreurs: {error_count}/{len(PREMIER_LEAGUE_TEAMS)}")
    ...
    print(f"\n✨ Refresh Premier League terminé!")
    
    return success_count == len(PREMIER_LEAGUE_TEAMS)

if __name__ == "__main__":
    try:
        success = refresh_premier_league()
