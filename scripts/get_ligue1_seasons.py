...
if response.status_code == 200:
    ...
    if 'data' in data:
        ...
        print(f"Nombre total de saisons trouvées: {len(seasons)}\n")
        ...
        for season in seasons_sorted[:10]:  # Afficher les 10 dernières
            ...
            # Identifier les saisons spécifiques
            if '2025' in name or '2026' in name:
                if is_current or not finished:
                    target_seasons['2025/2026'] = season_id
            elif '2024' in name or '2025' in name:
                if finished or '2024' in starting_at:
                    target_seasons['2024/2025'] = season_id
            elif '2023' in name or '2024' in name:
                if finished and '2023' in starting_at:
        ...
        print("\n" + "=" * 60)
        print("SAISONS IDENTIFIÉES POUR L'OM:")
        print("=" * 60)
        ...
        if target_seasons:
            season_ids = list(target_seasons.values())
            print(f"SEASON_IDS = {season_ids}  # IDs corrects pour Ligue 1")
            print(f"# Mapping: {target_seasons}")
        else:
