...
export default function ClubPagePremierLeague({ 
  clubId, 
  clubName, 
  leagueId, 
  leagueName,
  ...
  const team = premierLeagueTeams.find(t => t.slug === clubId);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black p-8">
        <div className="max-w-7xl mx-auto text-center text-white">
          ...
          <Link href="/premier-league" className="text-blue-400 hover:underline">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-950 via-gray-900 to-black">
      {/* Header */}
      <div className="relative bg-gradient-to-r from-purple-900 to-black p-8 shadow-2xl border-b border-purple-700">
        <Link 
          href={`/${leagueId}`}
      ...
      <div className="max-w-7xl mx-auto px-8 py-6">
        <div className="relative max-w-md mx-auto">
          <input
            ...
            className="w-full px-6 py-3 bg-gray-800 text-white rounded-full border border-purple-700 focus:border-purple-500 focus:outline-none pl-12"
          />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
                  {players.map((player) => {
                    ...
                    return (
                      <Link
                        key={player.id}
                        href={`/premier-league/${clubId}/${playerSlug}`}
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <div className="relative w-12 h-12">
                            ...
                            <div 
                              className={`w-12 h-12 bg-gradient-to-br from-purple-600 to-purple-800 rounded-full flex items-center justify-center text-white font-bold ${player.image ? 'hidden' : ''}`}
                              style={player.image ? { display: 'none' } : {}}
                            >
                              {player.jersey || player.name.charAt(0)}
                            </div>
                            {player.jersey && (
                              <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-black rounded-full flex items-center justify-center text-white text-xs font-bold border border-purple-600">
                                {player.jersey}
                              </div>
                            )}
                          </div>
