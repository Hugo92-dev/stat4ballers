...
class FinalRosterSync {
    constructor() {
        ...
        this.reportPath = path.join(__dirname, '../../data/db/sync-final-report-' + new Date().toISOString().split('T')[0] + '.json');
    ...
    async fetchPlayerCurrentTeams(playerId) {
        // Check cache first
        if (this.playerCurrentClubs.has(playerId)) {
            return this.playerCurrentClubs.get(playerId);
        }

        try {
            // Use the teams endpoint to get player's team history
            ...
            if (response.data?.data?.teams) {
                ...
                // Get all teams where the player currently plays (no end date or future end date)
                const currentTeams = teams
                    .filter(t => {
                        if (!t.end || t.end === null) return true;
                        const endDate = new Date(t.end);
                        return endDate > new Date(); // Future end date = still active
                    })
                    .map(t => t.team_id);
    ...
    async processPlayer(squadEntry, team, league) {
        ...
        // Check player's current teams
        const currentTeams = await this.fetchPlayerCurrentTeams(player.id);

        // Check if the player is actually in this team
        if (!currentTeams.includes(team.id)) {
            // Player is NOT in this team anymore
            ...
            return null; // Exclude this player from this team
        }
    ...
    async syncLeague(league) {
        ...
        for (const team of league.teams) {
            ...
            try {
                ...
                // Process each player with loan detection
                ...
                if (excludedCount > 0) {
                    console.log(`\n      🚫 Excluded ${excludedCount} loaned/transferred players from ${team.name}`);
                    this.report.leagues[league.slug].loanedPlayersExcluded += excludedCount;
                }
    ...
    async syncAll() {
        console.log('🚀 Starting FINAL roster synchronization with loan detection...');
    ...
    printSummary(allPlayers) {
        ...
        if (this.report.loanedPlayers.length > 0) {
            ...
            this.report.loanedPlayers.slice(0, 15).forEach((p, i) => {
                console.log(`   ${i + 1}. ${p.name} (was listed in ${p.squadTeam})`);
            });
        ...
        if (cornelius) {
            console.log(`   ⚠️  Cornelius still in ${cornelius.team.name}`);
        } else {
            console.log(`   ✅ Cornelius removed (not at Marseille anymore)`);
        }

        if (this.report.failedPlayerChecks.length > 0) {
            console.log(`\n⚠️  Failed to check ${this.report.failedPlayerChecks.length} players' current teams`);
        }
    }
}
...
// Main execution
async function main() {
    const syncer = new FinalRosterSync();

    try {
        await syncer.syncAll();
        console.log('\n✅ Final roster synchronization completed successfully!');
...
module.exports = FinalRosterSync;